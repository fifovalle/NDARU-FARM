...
const DaftarMenu = ({ setDaftarMenuAktif }) => {
  const [menuAktif, setMenuAktif] = useState("saranaPertanian");
      ...
      <div className="relative">
        <Typography
          ...
          className={`text-gray-500 font-bold cursor-pointer transition-all duration-300 ${
            menuAktif === "saranaPertanian" ? "text-gray-100" : ""
          }`}
          onClick={() => tanganiMenuSaatDiKlik("saranaPertanian")}
        >
          Sarana Pertanian
        </Typography>
        {menuAktif === "saranaPertanian" && (
